...
module.exports = function stringify (node, parent, options) {
  ...
  if (isVNode(node)) {
    ...
    if (~options.selfClosingTags.indexOf(node.tagName.toLowerCase())) {
      ...
      if (node.children && node.children.length) {
        for (var i=0, l=node.children.length; i<l; i++) {
          ...
          html.push(stringify(child, node, { invalidAttributes: options.invalidAttributes }));
        }
      }
    ...
    if (parent && parent.tagName === 'script') {
      ...
      var tree = node.view.tree;
      tree.properties['data-vdom-widget'] = node.view.widgetTag;
      Object.keys(node.view._widgetAttrs || {}).forEach(function (k) {
          var val = node.view._widgetAttrs[k].toString();
          k[0] = k[0].toUpperCase();
          k = 'data-' + k.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
          tree.properties[k] = val;
      });
      html.push(stringify(tree, null, { invalidAttributes: options.invalidAttributes }));
  }
